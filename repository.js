...
export async function createPostgresRepository({databaseUrl, dataDir, initial}) {
  ...
  const schema = [
    ...
    `create table if not exists io_federation_events(issuer text not null,event_id text not null,received_at timestamptz not null default now(),primary key(issuer,event_id))`
  ];
  ...
  async function load(){
    ...
    for(const [k,sql] of Object.entries({
      ...
      else if(k==='federationEvents') for(const r of rows) state[k][`${r.issuer}:${r.event_id}`]={issuer:r.issuer,eventId:r.event_id,receivedAt:Number(r.received_at)}
    }
    return state;
  }
