...
const initial={objects:{},graphs:{},events:[],invites:{},receipts:{},deliveries:{},outbox:{},peers:{}};
...
function federationDeliveriesFor(event){return Object.values(state.peers).map(peer=>({id:id(),kind:'federation',graphId:event.graphId||null,eventType:event.type,payload:{peerId:peer.id,envelope:federationEvent(event)},status:'pending',attempts:0,nextAttemptAt:Date.now()}));}
...
function federationEvent(event){const payload={protocol:'internet-objects',version:VERSION,issuer:federationIssuer(),event};return {...payload,signature:signFederationEnvelope(payload),signatureAlgorithm:'Ed25519',publicKey:federationPublicKeyPem()};}
...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');
 ...
 if(req.method==='POST'&&u.pathname==='/api/federation/register'){const b=await body(req);if(!b.url||!String(b.url).startsWith('http')||!b.publicKey)return json(res,400,{error:'url and publicKey are required'});const url=String(b.url).replace(/\/$/,'');let remote;try{const r=await fetch(`${url}/.well-known/internet-objects`);if(!r.ok)throw Error(`manifest HTTP ${r.status}`);remote=await r.json();}catch(e){return json(res,400,{error:`peer manifest unavailable: ${e.message}`});}if(!remote.publicKey)return json(res,400,{error:'peer manifest has no public key'});const pid=id();state.peers[pid]={id:pid,url,name:b.name||null,issuer:remote.issuer||url,publicKey:remote.publicKey,createdAt:Date.now(),lastSeenAt:Date.now(),payload:{id:pid,url,name:b.name||null,issuer:remote.issuer||url,publicKey:remote.publicKey}};if(repo.kind==='postgres')await repo.transaction(tx=>repo.putPeer(tx,state.peers[pid]));else save();return json(res,201,{peer:state.peers[pid]});}
 if(req.method==='POST'&&u.pathname==='/api/federation/ingest'){const b=await body(req);if(!b.event||b.protocol!=='internet-objects'||b.signatureAlgorithm!=='Ed25519'||!b.signature||!b.issuer)return json(res,400,{error:'invalid signed federation event'});const peer=Object.values(state.peers).find(p=>p.issuer===b.issuer);if(!peer)return json(res,403,{error:'unknown federation issuer'});const unsigned={protocol:b.protocol,version:b.version,issuer:b.issuer,event:b.event};if(!verifyFederationEnvelope(unsigned,b.signature,peer.publicKey))return json(res,401,{error:'invalid federation signature'});const eid=id();const e={id:eid,type:'federated_event',at:Date.now(),issuer:b.issuer,remoteEvent:b.event,verified:true};if(repo.kind==='postgres')await repo.transaction(tx=>tx.insertEvent(e));else{state.events.push(e);save()}peer.lastSeenAt=Date.now();if(repo.kind==='postgres')await repo.transaction(tx=>repo.putPeer(tx,peer));return json(res,202,{accepted:true,verified:true,eventId:eid});}
...
server.listen(process.env.PORT||3000,()=>console.log(`Internet Objects ${VERSION} listening on http://localhost:${process.env.PORT||3000}`));
